...
# False to predict, true to train and print the test accurency
is_train = True
...
with tf.Session() as sess:
    ...
    if is_train:
        ...
        saver.save(sess, 'mnistsess/model.cpkt')
        print('test accuracy %g' % (acc.eval(feed_dict={x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})))
    else:
        saver.restore(sess, 'mnistsess/model.cpkt')
        for i in [5,7,8]:
            img = get_img(str(i)+"test.jpg")
            predict = tf.argmax(y_fc2, 1)
            print("\(0^◇^0)/  I guess the number in the picture is %d" % sess.run(predict, feed_dict={x: img.reshape(-1, 784), keep_prob: 1.0}).item())
