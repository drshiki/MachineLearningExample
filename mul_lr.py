...
train_data = np.loadtxt("/mnt/hgfs/corpus/ex1data2.txt", delimiter=',')
...
#y_train = (y_train-np.mean(y_train, axis=0))/(np.max(y_train, axis=0)-np.min(y_train, axis=0))
#mean = np(X_train, aixs=0)
...
print(X_mean)
print(X_devision)
#print(session.run(tf.add(tf.matmul(X,W), b), feed_dict={X: [[1,2]],y:3}))
